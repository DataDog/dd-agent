...
def get_metrics(obj, ip, port, username, passwd, instance_name):
    ...
    log.print_log(obj, 'function: get_metrics   |input:- ip:%s,port:%s,user:%s,password:%s'%(ip, port, username, passwd))
    ...
    if username == 'n/s':
        ...
        node_latency = base.get_node_latency(ip, port, user=username, password=passwd)
        namespaces = base.get_namespaces(ip, port, user=username, password=passwd)
    ...
    if info != {}:
        ...
        for key, value in stats.iteritems():
            obj.gauge('aerospike.node.'+str(key), str(value), tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
            ...
            for key, value in node_latency.iteritems():
                ...
                for item in data_list:
                    for k,v in item.iteritems():
                        ...
                        tags_pct = ['latency_type:' + str(k), 'value_type:percentage', 'node:' + str(ip)+':'+str(port), 'name:' + str(instance_name)]
                        tags_value = ['latency_type:' + str(k), 'value_type:value', 'node:' + str(ip)+':'+str(port), 'name:' + str(instance_name)]
                        obj.gauge(metric_name, v['value'], tags_value)
                        obj.gauge(metric_name, v['pct'], tags_pct)

        if free_disk_stats != 'n/s':
            obj.gauge('aerospike.disk_usage_free', free_disk_stats, tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        if total_disk_stats != 'n/s':
            obj.gauge('aerospike.disk_usage_total', total_disk_stats, tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        if free_memory_stats != 'n/s':
            obj.gauge('aerospike.memory_usage_free', free_memory_stats, tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        if total_memory_stats != 'n/s':
            obj.gauge('aerospike.memory_usage_total', total_memory_stats, tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        if no_of_nodes is not None:
            obj.gauge('aerospike.cluster_size', no_of_nodes, tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        else:
            log.print_log(obj, 'Number of Nodes are None!', error_flag=True)
        if read_tps['y'] is not None:
            obj.gauge('aerospike.successful_read_tps', read_tps['y'], tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        if read_tps['secondary'] is not None:
            obj.gauge('aerospike.total_read_tps', read_tps['secondary'], tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        if write_tps['y'] is not None:
            obj.gauge('aerospike.successful_write_tps', write_tps['y'], tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
        if write_tps['secondary'] is not None:
            obj.gauge('aerospike.total_write_tps', write_tps['secondary'], tags=['node:'+ str(ip) + ':'+ str(port), 'name:' + str(instance_name)])
    ...
    if namespaces not in ERROR_CODES:
        ...
        for ns in namespaces:
            if username == 'n/s':
                ...
                ns_stats = base.get_namespace_statistics(ip, port, ns, user=username, password=passwd)
            if ns_stats not in ERROR_CODES:
                for key, value in ns_stats.iteritems():
                    metric_name = 'aerospike.namespace.' + str(ns) + '.' + str(key)
                    obj.gauge(metric_name, str(value), tags=['node:'+ str(ip) + ':'+ str(port),'namespace:' + str(ns), 'name:' + str(instance_name)])
                ns_alerts = base.get_namespace_alerts(ns, ns_stats, str(ip)+':'+str(port))
