...
try:
    from checks.libs.wmi_old.sampler import WMISampler
except Exception:
...
class Processes(Check):
    def __init__(self, logger):
        ...
        self.gauge('old.system.proc.queue_length')
        self.gauge('old.system.proc.count')
    ...
    def check(self, agentConfig):
        ...
        if processor_queue_length is not None:
            self.save_sample('old.system.proc.queue_length', processor_queue_length)
        if processes is not None:
            self.save_sample('old.system.proc.count', processes)
...
class Memory(Check):
    def __init__(self, logger):
        ...
        self.gauge('old.system.mem.nonpaged')
        # usable = free + cached
        self.gauge('old.system.mem.usable')
        self.gauge('old.system.mem.pct_usable')
    ...
    def check(self, agentConfig):
        ...
        if total_visible_memory_size is not None and free_physical_memory is not None:
            ...
            self.save_sample('old.system.mem.total', total)
            self.save_sample('old.system.mem.free', free)
            self.save_sample('old.system.mem.used', total - free)
        ...
        if cache_bytes is not None:
            ...
            self.save_sample('old.system.mem.cached', cached)
        if committed_bytes is not None:
            self.save_sample('old.system.mem.committed', int(committed_bytes) / B2MB)
        if pool_paged_bytes is not None:
            self.save_sample('old.system.mem.paged', int(pool_paged_bytes) / B2MB)
        if pool_non_paged_bytes is not None:
            self.save_sample('old.system.mem.nonpaged', int(pool_non_paged_bytes) / B2MB)

        usable = free + cached
        self.save_sample('old.system.mem.usable', usable)
        if total > 0:
            pct_usable = float(usable) / total
            self.save_sample('old.system.mem.pct_usable', pct_usable)
...
class Cpu(Check):
    def __init__(self, logger):
        ...
        self.counter('old.system.cpu.user')
        self.counter('old.system.cpu.idle')
        self.gauge('old.system.cpu.interrupt')
        self.counter('old.system.cpu.system')
    ...
    def check(self, agentConfig):
        ...
        if cpu_interrupt is not None:
            self.save_sample('old.system.cpu.interrupt', cpu_interrupt)

        cpu_percent = psutil.cpu_times()

        self.save_sample('old.system.cpu.user', 100 * cpu_percent.user / psutil.cpu_count())
        self.save_sample('old.system.cpu.idle', 100 * cpu_percent.idle / psutil.cpu_count())
        self.save_sample('old.system.cpu.system', 100 * cpu_percent.system / psutil.cpu_count())
...
class Network(Check):
    def __init__(self, logger):
        ...
        self.gauge('old.system.net.bytes_rcvd')
        self.gauge('old.system.net.bytes_sent')
    ...
    def check(self, agentConfig):
        ...
        for iface in self.wmi_sampler:
            ...
            if bytes_received_per_sec is not None:
                self.save_sample('old.system.net.bytes_rcvd', bytes_received_per_sec,
                                 device_name=name)
            if bytes_sent_per_sec is not None:
                self.save_sample('old.system.net.bytes_sent', bytes_sent_per_sec,
                                 device_name=name)
        return self.get_metrics()
...
class IO(Check):
    def __init__(self, logger):
        ...
        self.gauge('old.system.io.wkb_s')
        self.gauge('old.system.io.w_s')
        self.gauge('old.system.io.rkb_s')
        self.gauge('old.system.io.r_s')
        self.gauge('old.system.io.avg_q_sz')
    ...
    def check(self, agentConfig):
        ...
        for device in self.wmi_sampler:
            ...
            if disk_write_bytes_per_sec is not None:
                self.save_sample('old.system.io.wkb_s', int(disk_write_bytes_per_sec) / B2KB,
                                 device_name=name)
            if disk_writes_per_sec is not None:
                self.save_sample('old.system.io.w_s', int(disk_writes_per_sec),
                                 device_name=name)
            if disk_read_bytes_per_sec is not None:
                self.save_sample('old.system.io.rkb_s', int(disk_read_bytes_per_sec) / B2KB,
                                 device_name=name)
            if disk_reads_per_sec is not None:
                self.save_sample('old.system.io.r_s', int(disk_reads_per_sec),
                                 device_name=name)
            if current_disk_queue_length is not None:
                self.save_sample('old.system.io.avg_q_sz', current_disk_queue_length,
                                 device_name=name)
        return self.get_metrics()
