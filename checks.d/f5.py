...
class F5Check(AgentCheck):
    ...
    def get_session(self, host, user, authPassword, privPassword, port=161):
        ...
        try:                                                        # Try to
            ...
            self.log.error("Could not establish session")           # log_level:error
            self.log.error(e)                                       # log_level:error
            ...
            try:                                                                # Attempt to
                ...
                if "nosuch" in item.snmp_type.lower():                          # If it returns nosuchinstance or nosuchobject
                    ...
                    try:                                                        # Attempt to
                        ...
                        self.log.error("Could not convert item: %s", str(item)) # log_level:error
                        self.log.error(e)                                       # log_level:error
    ...
    def get_snmp_custom_metrics(self, session, CUSTOM_METRICS):
        ...
        for e in CUSTOM_METRICS:                                            # For every defined custom metric
            ...
            tmp = snmp_obj.split(":")                                       # Split the SNMP Object Name by ':'
            name = tmp[len(tmp)-1]                                          # And take the last value (Eliminate MIB Module from name)
    ...
    def check(self, instance):
        ...
        try:                                                                                    # Attempt to
            ...
            self.log.warning("Skipping instance check, Reason: No %s defined", e)               # log_level:warning
            sys.exit(1)                                                                         # Exit with an exit code of 1
