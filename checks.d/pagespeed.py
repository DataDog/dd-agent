...
class Pagespeed(AgentCheck):
    ...
    * child_shutdown_count
    ...
    def check(self, instance):
        ...
        url = self.assumed_url.get(instance['pagespeed_status_url'], instance['pagespeed_status_url'])
        ...
        if 'pagespeed_status_user' in instance and 'pagespeed_status_password' in instance:
            auth_str = '%s:%s' % (instance['pagespeed_status_user'], instance['pagespeed_status_password'])
        ...
        if metric_count == 0:
            if self.assumed_url.get(instance['pagespeed_status_url'], None) is None and url[-5:] != '?auto':
                self.assumed_url[instance['pagespeed_status_url']]= '%s?auto' % url
                self.warning("Assuming url was not correct. Trying to add ?auto suffix to the url")
                self.check(instance)
            else:
                raise Exception("No metrics were fetched for this instance. Make sure that %s is the proper url." % instance['pagespeed_status_url'])

    @staticmethod
    def parse_agent_config(agentConfig):
        if not agentConfig.get('pagespeed_status_url'):
            return False

        return {
            'instances': [{'pagespeed_status_url': agentConfig.get('pagespeed_status_url')}]
        }
