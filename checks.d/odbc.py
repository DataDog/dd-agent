...
class ODBCConnectionError(Exception):
    """
    Exception raised for ODBC instance connection issues
    """
    pass
...
class SQLServer(AgentCheck):
    ...
    SERVICE_CHECK_NAME = 'odbcsql.can_connect'
    # FIXME: 6.x, set default to 5s (like every check)
    DEFAULT_COMMAND_TIMEOUT = 30
    DEFAULT_DATABASE = 'master'
    DEFAULT_DRIVER = 'SQL Server'
    METRICS = [
        ('odbcsql.buffer.cache_hit_ratio', 'Buffer cache hit ratio', ''),  # RAW_LARGE_FRACTION
        ('odbcsql.buffer.page_life_expectancy', 'Page life expectancy', ''),  # LARGE_RAWCOUNT
        ('odbcsql.stats.batch_requests', 'Batch Requests/sec', ''),  # BULK_COUNT
        ('odbcsql.stats.sql_compilations', 'SQL Compilations/sec', ''),  # BULK_COUNT
        ('odbcsql.stats.sql_recompilations', 'SQL Re-Compilations/sec', ''),  # BULK_COUNT
        ('odbcsql.stats.connections', 'User Connections', ''),  # LARGE_RAWCOUNT
        ('odbcsql.stats.lock_waits', 'Lock Waits/sec', '_Total'),  # BULK_COUNT
        ('odbcsql.access.page_splits', 'Page Splits/sec', ''),  # BULK_COUNT
        ('odbcsql.stats.procs_blocked', 'Processes blocked', ''),  # LARGE_RAWCOUNT
        ('odbcsql.buffer.checkpoint_pages', 'Checkpoint pages/sec', '')  # BULK_COUNT
    ]
    ...
    def _make_metric_list_to_collect(self, instance, custom_metrics):
        ...
        for row in custom_metrics:
            ...
            if not db_table in self.valid_tables:
    ...
    def open_db_connections(self, instance):
        ...
        try:
            ...
            cxn_failure_exp = ODBCConnectionError("%s \n %s" % (message, tracebk))
...
class SqlOsWaitStat(SqlServerMetric):
    def fetch_metric(self, cursor, tags):
        ...
        if len(rows) == 0:
            self.log.debug("No rows in %s" % self.sql_name)
...
class SqlSimpleMetric(SqlServerMetric):
    ...
    def fetch_metric(self, cursor, tags):
        ...
        rows = cursor.fetchall()
        if len(rows) == 0:
            self.log.debug("No rows in %s" % self.instance)
        else:
            self.log.debug("%d rows in instance %s" % ( len(rows), self.instance))
