...
class JMXFetch(Daemon):
    """Daemon around JMXFetch process"""
    def __init__(self, confd_path, agentConfig, pid_file=None, autorestart=None):
    ...
    def run(self):
        ...
        self.initialize(JMX_COLLECT_COMMAND)

    def initialize(self, command=None, checks_list=None, reporter=None):
        try:
...
def main(config_path=None):
    ...
    jmx_daemon = JMXFetch(confd_path, agentConfig, pid_file=pid_file.get_path(),
                          autorestart=autorestart)

    # If no args were passed in, do nothing
    if not args:
        jmx_daemon.start(foreground=True)
        return 0
    else:
        command = args[0]

        if command == "start":
            jmx_daemon.start(use_chdir=False)
        elif command == "stop":
            jmx_daemon.stop()
