  ...
  namespace :mysql do |flavor|
      ...
      unless Dir.exist? File.expand_path(mysql_rootdir)
        ...
        sh %(cd $VOLATILE_DIR/mysql &&\
             cmake -DCMAKE_INSTALL_PREFIX=#{mysql_rootdir} -DMYSQL_DATADIR=#{mysql_rootdir}/data \
               -DMYSQL_UNIX_ADDR=#{mysql_rootdir}/mysql.sock -DDOWNLOAD_BOOST=1 -DWITH_BOOST=VOLATILE_DIR/mysql/boost &&\
             make -j $CONCURRENCY &&\
             make install)
      end
    end
    ...
    task before_script: ['ci:common:before_script'] do
      if Gem::Version.new("#{mysql_version}") >= Gem::Version.new('5.7.6')
        sh %(#{mysql_rootdir}/bin/mysqld --no-defaults --user=$USER --initialize-insecure --basedir=#{mysql_rootdir} --datadir=#{mysql_rootdir}/data)
      else
        ...
        sh %(#{execute_file} --no-defaults --user=$USER #{insecure_option} --basedir=#{mysql_rootdir} --datadir=#{mysql_rootdir}/data)
      end
