  ...
  namespace :kong do |flavor|
    ...
    task install: ['ci:common:install'] do
      unless Dir.exist? File.expand_path(kong_rootdir)
        ...
        ENV['PATH'] = "#{ENV['LUAJIT_DIR']}/bin:#{ENV['LUAJIT_DIR']}/include/#{ENV['LUA_VERSION']}:#{ENV['LUAROCKS_DIR']}/bin:\
            #{ENV['OPENRESTY_DIR']}/nginx/sbin:#{ENV['SERF_DIR']}:#{ENV['DNSMASQ_DIR']}/usr/local/sbin:#{ENV['PATH']}"
        sh %(curl -s -L -o $VOLATILE_DIR/apache-cassandra-2.2.4-bin.tar.gz\
             http://apache.arvixe.com/cassandra/2.2.4/apache-cassandra-2.2.4-bin.tar.gz)
        sh %(mkdir -p #{cassandra_rootdir})
        sh %(tar zxf $VOLATILE_DIR/apache-cassandra-2.2.4-bin.tar.gz\
