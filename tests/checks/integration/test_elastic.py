...
class TestElastic(AgentCheckTest):
    ...
    def test_pshard_metrics(self):
        ...
        for m_name, desc in PRIMARY_SHARD_METRICS.iteritems():
            if desc[0] == "gauge":
                self.assertMetric(m_name, count=1)

        # Our pshard metrics are getting sent, let's check that they're accurate though
        self.assertMetric('elasticsearch.primaries.docs.count', value=2)
        self.assertMetric('elasticsearch.docs.count', value=4)
