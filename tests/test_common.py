...
from checks.collector import Collector, AgentPayload
...
class TestMetadataService(unittest.TestCase):
    ...
    def test_metadata_length(self):
        # Fake config. Instance 2 will not generate any meta
        config = {'instances': [{'metadata': True}, {'metadata': False}]}
        agentConfig = {
        }

        instances = config.get('instances')
        check = TestMetadataService.FakeCheck("fake_check", config, agentConfig, instances)
        check.run()
