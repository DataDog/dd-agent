...
tar -xz -C "$DD_HOME/agent" -f "$DD_HOME/agent.tar.gz"
mv $DD_HOME/agent/*/* $DD_HOME/agent/
...
if [ "$DD_SKIP_INTEGRATIONS" = "1" ]; then
  ...
  tar -xz -C "$DD_HOME/integrations" -f "$DD_HOME/integrations.tar.gz"
  mv $DD_HOME/integrations/*/* $DD_HOME/integrations/
